...
async function testChromaDB(documents,ids) {
...
async function queryChromaDB(queryTexts, nResults) {
  const queryEmbeddings = await Promise.all(queryTexts.map(generateEmbedding));
...
export { testChromaDB, testQueryChromaDB, queryChromaDB, generateEmbedding }